...
## Main entry — returns Vector3 spawn position (y = terrain + 0.15).
...
static func _city_center(city_plan: CityPlan, world_plan: WorldPlan) -> Vector3:
	if city_plan != null and city_plan.has_method("find_spawn_point"):
		...
		var h: float = world_plan.terrain_height_at(p2) if world_plan != null else 0.0
		return Vector3(p2.x, h + 0.15, p2.y)
	return Vector3(-96.5, 0.15, -44.0)
...
static func _city_gate(world_plan: WorldPlan, city_plan: CityPlan) -> Vector3:
	if world_plan != null:
		...
		if not gates.is_empty():
			...
			var h: float = world_plan.terrain_height_at(outside)
			if world_plan.water_body_at(outside) == &"":
				return Vector3(outside.x, h + 0.15, outside.y)
			return Vector3(p.x, world_plan.terrain_height_at(p) + 0.15, p.y)
	return _city_center(city_plan, world_plan)
...
static func _settlement_kind(kind: StringName, world_plan: WorldPlan, city_plan: CityPlan) -> Vector3:
	...
	for a in filtered:
		...
		if not gates.is_empty():
			...
			return Vector3(gp.x, world_plan.terrain_height_at(gp) + 0.15, gp.y)
		return Vector3(p.x, h + 0.15, p.y)
	var p0: Vector2 = filtered[0]["center"] as Vector2
	return Vector3(p0.x, world_plan.terrain_height_at(p0) + 0.15, p0.y)
...
static func _forest_spawn(world_plan: WorldPlan, city_plan: CityPlan) -> Vector3:
	...
	for _i in 200:
		...
		if biome != &"deciduous_forest" and biome != &"mixed_upland_forest":
			continue
		var slope: float = world_plan.terrain_slope_at(p)
...
static func _river_bank_spawn(world_plan: WorldPlan, city_plan: CityPlan) -> Vector3:
	...
	for _i in 300:
		...
		var bank_offset: float = w * 0.5 + WorldConstants.BANK_W * 0.5 + rng.randf_range(2.0, 8.0)
	...
	if not cands.is_empty():
		...
		for p in cands:
			if p.length() < WorldConstants.URBAN_OUTER_M:
				continue
			var h: float = world_plan.terrain_height_at(p as Vector2)
			# bridge deck is terrain + BRIDGE_DECK_LIFT, but player stands on deck
			return Vector3((p as Vector2).x, h + 0.15 + WorldConstants.BRIDGE_DECK_LIFT_M, (p as Vector2).y)
		var p0: Vector2 = cands[0] as Vector2
		return Vector3(p0.x, world_plan.terrain_height_at(p0) + 0.15 + WorldConstants.BRIDGE_DECK_LIFT_M, p0.y)
	return _river_bank_spawn(world_plan, city_plan)
...
static func _hill_spawn(world_plan: WorldPlan, city_plan: CityPlan) -> Vector3:
	...
	if best_p != Vector2.ZERO and best_h > 18.0:
		return Vector3(best_p.x, best_h + 0.15, best_p.y)
...
static func _quarry_spawn(world_plan: WorldPlan, city_plan: CityPlan) -> Vector3:
	...
	for _i in 400:
		...
		if world_plan.water_body_at(p) != &"":
			continue
		var biome: StringName = world_plan.biome_at(p)
		var quarry: bool = biome == &"rocky_quarry" or world_plan.is_quarry(p)
		var suit: float = world_plan.quarry_suitability_at(p) if world_plan.has_method("quarry_suitability_at") else 0.0
		if not quarry and suit < WorldConstants.QUARRY_SUITABILITY_THRESHOLD - 0.1:
			continue
		var h: float = world_plan.terrain_height_at(p)
		return Vector3(p.x, h + 0.15, p.y)
	return _hill_spawn(world_plan, city_plan)
...
static func _cave_spawn(world_plan: WorldPlan, city_plan: CityPlan) -> Vector3:
	...
	if best_p != Vector2.ZERO and best_pot > 0.45:
		return Vector3(best_p.x, world_plan.terrain_height_at(best_p) + 0.15, best_p.y)
	return _quarry_spawn(world_plan, city_plan)
