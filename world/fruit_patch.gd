class_name FruitPatch
extends Area3D
## Orchard fruit patch: hedgerow-bounded canopy row harvest, renewable fruit.
## Prompt "Harvest apples (Apple x1)" when grown else "Growing — ready in N days"
## After harvest: "Picked clean — regrows in 3 days" and regrows after 3 days via GameClock.
## ACTIVE-only via ChunkManager: monitorable/enabled toggled, no collider counted.

var fruit_id: String = ""
var parcel_id: String = ""
var fruit_kind: StringName = &"apple"
...
func setup(data: Dictionary) -> void:
	fruit_id = String(data.get("id", data.get("parcel_id", name)))
	parcel_id = String(data.get("parcel_id", fruit_id))
	if data.has("fruit_kind"):
		fruit_kind = StringName(str(data["fruit_kind"]))
	elif data.has("kind"):
		fruit_kind = StringName(str(data["kind"]))
	elif data.has("crop_kind"):
		fruit_kind = StringName(str(data["crop_kind"]))
...
func _is_grown() -> bool:
	return _cur_day() >= planted_day + WorldConstants.FRUIT_GROW_DAYS
...
func _try_regrow() -> void:
	...
	if depleted_at_day < 0:
		return
	if _cur_day() >= depleted_at_day + WorldConstants.FRUIT_REGROW_DAYS:
		depleted = false
		depleted_at_day = -1
		_update_prompt()
...
func _on_interacted(player: Node3D) -> void:
	...
	for mgr in get_tree().get_nodes_in_group(&"chunk_manager"):
		if mgr.has_method("_record_fruit_patch"):
			mgr.call("_record_fruit_patch", coord, fruit_id, save_state())
		elif mgr.has_method("record_fruit_patch_state"):
			mgr.call("record_fruit_patch_state", coord, fruit_id, save_state())
...
func _update_prompt() -> void:
	...
	_try_regrow()
	if depleted:
		interactable.prompt = "Picked clean \u2014 regrows in 3 days"
		interactable.enabled = false
		monitorable = false
		return
	if not _is_grown():
		var days_left: int = (planted_day + WorldConstants.FRUIT_GROW_DAYS) - _cur_day()
		if days_left < 0:
			days_left = 0
	...
	for id in contents.keys():
		...
		break
	if label == "":
		label = "%s x1" % ItemDB.item_name(fruit_kind)
	var fruit_label := String(fruit_kind).capitalize()
	interactable.prompt = "Harvest %s (%s)" % [String(fruit_kind), label]
...
func save_state() -> Dictionary:
	...
	return {"depleted": depleted, "depleted_at_day": depleted_at_day, "contents": out_contents, "planted_day": planted_day, "fruit_kind": String(fruit_kind)}
