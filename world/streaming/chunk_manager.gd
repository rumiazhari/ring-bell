...
var _orchard_parcels_total := 0
var _fruit_patches_total := 0
var _orchard_vertices_total := 0
var _orchard_triangles_total := 0
var _orchard_instances_total := 0
var _orchard_hedgerow_total := 0
...
func reset_stream() -> void:
	...
	_orchard_parcels_total = 0
	_fruit_patches_total = 0
	_orchard_vertices_total = 0
	_orchard_triangles_total = 0
	_orchard_instances_total = 0
	_orchard_hedgerow_total = 0
...
func _materialize(coord: Vector2i, batcher: MeshBatcher, terrain_manifest: Dictionary, gen_ms: float,
	...
	if not biome_manifest.is_empty():
		...
		if _records.has(coord):
			...
			# Apply fruit patch deltas before materialize (persistence-first) and handle regrow via GameClock
			if _records.has(coord):
				var b_deltas_f: Dictionary = _records[coord].get("deltas", {}) as Dictionary
				var fr_deltas: Dictionary = b_deltas_f.get("fruit_patches", {}) as Dictionary
				if not fr_deltas.is_empty():
					var cur_day_fr: int = GameClock.get_day() if GameClock != null else 1
					var patched_fr := {}
					for fid in fr_deltas.keys():
						var fs: Dictionary = fr_deltas[fid] as Dictionary
						var dep: bool = bool(fs.get("depleted", false))
						var dep_day: int = int(fs.get("depleted_at_day", -1))
						if dep and cur_day_fr >= dep_day + WorldConstants.FRUIT_REGROW_DAYS:
							continue
						patched_fr[fid] = fs
					if patched_fr.size() != fr_deltas.size():
						b_deltas_f["fruit_patches"] = patched_fr
						_records[coord]["deltas"] = b_deltas_f
					var fruit_manifests: Array = biome_manifest.get("fruit_patch_manifests", []) as Array
					var patched_fruit_manifests: Array[Dictionary] = []
					for fm in fruit_manifests:
						var fd: Dictionary = fm as Dictionary
						var fid2: String = String(fd.get("id", ""))
						if patched_fr.has(fid2):
							var saved_f: Dictionary = patched_fr[fid2] as Dictionary
							fd = fd.duplicate()
							fd["depleted"] = bool(saved_f.get("depleted", false))
							fd["depleted_at_day"] = int(saved_f.get("depleted_at_day", -1))
							if saved_f.has("contents"):
								var new_contents_f: Dictionary = {}
								var sc_f: Dictionary = saved_f["contents"] as Dictionary
								for k in sc_f.keys():
									new_contents_f[StringName(str(k))] = int(sc_f[k])
								fd["contents"] = new_contents_f
							if saved_f.has("fruit_kind"):
								fd["fruit_kind"] = StringName(str(saved_f["fruit_kind"]))
							elif saved_f.has("kind"):
								fd["fruit_kind"] = StringName(str(saved_f["kind"]))
						patched_fruit_manifests.append(fd)
					biome_manifest["fruit_patch_manifests"] = patched_fruit_manifests
		...
		if chunk_node_b != null:
			...
			# Apply fruit patch depleted states after materialize (re-apply deltas before interactability)
			if _records.has(coord):
				var deltas_f2: Dictionary = _records[coord].get("deltas", {}) as Dictionary
				var fr_deltas2: Dictionary = deltas_f2.get("fruit_patches", {}) as Dictionary
				if not fr_deltas2.is_empty():
					_apply_fruit_patch_states(chunk_node_b, coord, fr_deltas2)
			# ACTIVE-only FruitPatch: disable when warm
			if not include_collision:
				_set_fruit_patches_enabled(chunk_node_b, coord, false)
			else:
				_set_fruit_patches_enabled(chunk_node_b, coord, true)
	...
	if not rural_manifest.is_empty():
		...
		if chunk_node_ru != null:
			...
			if _records.has(coord):
				...
				var g_deltas2: Dictionary = deltas2.get("granaries", {}) as Dictionary
				if not g_deltas2.is_empty():
					_apply_granary_states(chunk_node_ru, coord, g_deltas2)
			# ACTIVE-only rural physics: disable warm rural colliders (visual retained) and crate/well/forage/hearth interactability
			if not include_collision:
				...
				_set_rural_granary_enabled(chunk_node_ru, coord, false)
			else:
	...
	var orchard_parcels := int(bstats.get("orchard_parcels", 0))
	var fruit_patches := int(bstats.get("fruit_patches", 0))
	var orchard_vertices := int(bstats.get("orchard_vertices", 0))
	var orchard_triangles := int(bstats.get("orchard_triangles", 0))
	var orchard_hedgerow := int(bstats.get("orchard_hedgerow", 0))
	var orchard_instances := int(bstats.get("orchard_instances", 0))
	...
	_chunks[coord] = {
		...
		"orchard_parcels": orchard_parcels,
		"fruit_patches": fruit_patches,
		"orchard_vertices": orchard_vertices,
		"orchard_triangles": orchard_triangles,
		"orchard_hedgerow": orchard_hedgerow,
		"orchard_instances": orchard_instances,
		"orchard_parcels_active": orchard_parcels if include_collision else 0,
		"fruit_patches_active": fruit_patches if include_collision else 0,
		...
		"rural_granaries_active": rural_granaries if include_collision else 0,
		...
		"rural_hearths_active": rural_hearths if include_collision else 0,
	...
	_orchard_parcels_total += orchard_parcels
	_fruit_patches_total += fruit_patches
	_orchard_vertices_total += orchard_vertices
	_orchard_triangles_total += orchard_triangles
	_orchard_instances_total += orchard_instances
	_orchard_hedgerow_total += orchard_hedgerow
...
func _unload_far(desired: Dictionary, pc: Vector2i) -> void:
	...
	for c in doomed:
		...
		_orchard_parcels_total -= int(rec.get("orchard_parcels", 0))
		_fruit_patches_total -= int(rec.get("fruit_patches", 0))
		_orchard_vertices_total -= int(rec.get("orchard_vertices", 0))
		_orchard_triangles_total -= int(rec.get("orchard_triangles", 0))
		_orchard_instances_total -= int(rec.get("orchard_instances", 0))
		_orchard_hedgerow_total -= int(rec.get("orchard_hedgerow", 0))
...
func rural_granaries_active_count() -> int:
	var n := 0
	for v in _chunks.values():
		if v.get("state", "") == &"active":
			n += int(v.get("rural_granaries_active", v.get("rural_granaries", 0)))
	return n

func rural_granaries_total_count() -> int:
	return _rural_granaries_total

func _set_rural_granary_enabled(parent: Node, coord: Vector2i, enabled: bool) -> void:
	if parent == null or not is_instance_valid(parent):
		return
	var rural: Node = null
	if String(parent.name).begins_with("Rural_"):
	...
	if rural == null or not is_instance_valid(rural):
		return
	var children: Array = rural.get_children().duplicate()
	for child: Variant in children:
		if String(child.name).begins_with("rural_granary_"):
			if not is_instance_valid(child):
				continue
			if child.is_queued_for_deletion() or not child.is_inside_tree():
				continue
			if child.has_method("set_active_enabled"):
...
func _set_rural_wells_enabled(rural_parent: Node, coord: Vector2i, enabled: bool) -> void:
	...
	for child in rural_node.get_children():
		if child is Well:
			...
			if is_instance_valid(well) and not well.is_queued_for_deletion() and well.is_inside_tree():
				# P4.5 unified: Well is Area3D, no collider; ACTIVE-only via monitorable+enabled
				if well.has_method("set_active_enabled"):
					well.call("set_active_enabled", enabled)
				else:
					well.monitorable = enabled and not well.depleted if "depleted" in well else enabled
...
func _set_rural_forage_enabled(rural_parent: Node, coord: Vector2i, enabled: bool) -> void:
	...
	for child in rural_node.get_children():
		if child is ForagePatch:
			...
			if is_instance_valid(patch) and not patch.is_queued_for_deletion() and patch.is_inside_tree():
...
func _set_rural_hearth_enabled(rural_parent: Node, coord: Vector2i, enabled: bool) -> void:
	var rural_node: Node = rural_parent
	if rural_parent.name.begins_with("Chunk_"):
		rural_node = rural_parent.get_node_or_null(NodePath("Rural_%d_%d" % [coord.x, coord.y]))
		if rural_node == null:
	...
	for child in rural_node.get_children():
		if child is Stove:
			var stove: Stove = child as Stove
			if is_instance_valid(stove) and not stove.is_queued_for_deletion() and stove.is_inside_tree():
				if stove.has_method("set_active_enabled"):
					stove.call("set_active_enabled", enabled)
				else:
					stove.monitorable = enabled
					var inter = stove.get("interactable")
					if inter != null and is_instance_valid(inter):
						inter.enabled = enabled
		elif child is Bed:
			var bed: Bed = child as Bed
			if is_instance_valid(bed) and not bed.is_queued_for_deletion() and bed.is_inside_tree():
				if bed.has_method("set_active_enabled"):
					bed.call("set_active_enabled", enabled)
				else:
					bed.monitorable = enabled
...
func _set_fruit_patches_enabled(biome_parent: Node, coord: Vector2i, enabled: bool) -> void:
	var biome_node: Node = biome_parent
	if biome_parent.name.begins_with("Chunk_"):
		biome_node = biome_parent.get_node_or_null(NodePath("Biome_%d_%d" % [coord.x, coord.y]))
		if biome_node == null:
	...
	var children: Array = biome_node.get_children().duplicate()
	for child: Variant in children:
		if child is FruitPatch:
			var patch: FruitPatch = child as FruitPatch
			if not is_instance_valid(patch):
				continue
			if patch.is_queued_for_deletion() or not patch.is_inside_tree():
				continue
...
func _apply_fruit_patch_states(parent: Node, coord: Vector2i, fruit_patches: Dictionary) -> void:
	var biome_node: Node = parent.get_node_or_null(NodePath("Biome_%d_%d" % [coord.x, coord.y]))
	if biome_node == null:
		if parent.name.begins_with("Biome_"):
			biome_node = parent
	...
	for child in biome_node.get_children():
		if child is FruitPatch:
			...
			if fruit_patches.has(fid):
				(child as FruitPatch).load_state(fruit_patches[fid] as Dictionary)

func _record_fruit_patch(coord: Vector2i, fruit_id: String, state: Dictionary) -> void:
	if fruit_id == "":
		return
	note_discovered(coord)
	var fp: Dictionary = _records[coord]["deltas"].get("fruit_patches", {})
	fp[fruit_id] = state.duplicate()
	_records[coord]["deltas"]["fruit_patches"] = fp

func _snapshot_resident_fruit_patches() -> void:
	for c: Vector2i in _chunks:
		var node := get_node_or_null(NodePath("Chunk_%d_%d" % [c.x, c.y]))
		if node == null:
			continue
		_collect_fruit_patches_recursive(node, c)

func _collect_fruit_patches_recursive(n: Node, coord: Vector2i) -> void:
	for child in n.get_children():
		if child is FruitPatch and not (child as FruitPatch).is_queued_for_deletion():
			if String(child.name).begins_with("fruit_"):
				var st: Dictionary = (child as FruitPatch).save_state()
				_record_fruit_patch(coord, String(child.name), st)
		if child.get_child_count() > 0:
			_collect_fruit_patches_recursive(child, coord)

func orchard_parcels_active_count() -> int:
	var n := 0
	for v in _chunks.values():
		if v.get("state", "") == &"active":
			n += int(v.get("orchard_parcels", 0))
	return n

func fruit_patches_active_count() -> int:
	var n := 0
	for v in _chunks.values():
		if v.get("state", "") == &"active":
			n += int(v.get("fruit_patches", 0))
	return n

func orchard_parcels_total() -> int:
	return _orchard_parcels_total

func fruit_patches_total() -> int:
	return _fruit_patches_total
...
func _apply_granary_states(parent: Node, coord: Vector2i, granaries: Dictionary) -> void:
	var rural_node: Node = parent.get_node_or_null(NodePath("Rural_%d_%d" % [coord.x, coord.y]))
	if rural_node == null:
		if parent.name.begins_with("Rural_"):
			rural_node = parent
	...
	for child in rural_node.get_children():
		if String(child.name).begins_with("rural_granary_"):
			var gid: String = String(child.name)
			if granaries.has(gid):
				child.call("load_state", granaries[gid] as Dictionary)

func _record_granary(coord: Vector2i, granary_id: String, state: Dictionary) -> void:
	if granary_id == "":
		return
	note_discovered(coord)
	var granaries: Dictionary = _records[coord]["deltas"].get("granaries", {})
	granaries[granary_id] = state.duplicate()
	_records[coord]["deltas"]["granaries"] = granaries

func _snapshot_resident_granaries() -> void:
	for c: Vector2i in _chunks:
		var node := get_node_or_null(NodePath("Chunk_%d_%d" % [c.x, c.y]))
		if node == null:
			continue
		_collect_granaries_recursive(node, c)

func _collect_granaries_recursive(n: Node, coord: Vector2i) -> void:
	for child in n.get_children():
		if String(child.name).begins_with("rural_granary_") and not child.is_queued_for_deletion():
			var st: Dictionary = child.call("save_state") as Dictionary
			_record_granary(coord, String(child.name), st)
		if child.get_child_count() > 0:
			_collect_granaries_recursive(child, coord)
