...
STATUS: ACTIVE
LAST_ITER: 39
UPDATED: 2026-08-27 (JST, cron run) — iter 39

## Current goal
Iter 39: Phase Y Prague facade drainpipes + eave gutters — vertical zinc/copper downpipes (DRAIN_T 0.06, 62% chance) running full facade height + horizontal eave gutters (0.07×0.08) at roofline, pressed as thin visual planes 0.05 outside historic long facades (>=5.0), deterministic per WorldSeed drainpipe, gated historic, zinc 5a6d6e / copper 6a7a5e patina; BuildingBuilder._facade_drainpipes via WorldSeed, visual-only, no collision; citytest + smoke green (1 new assertion).
