...
STATUS: ACTIVE
LAST_ITER: 43
UPDATED: 2026-08-27 (JST, cron run) — iter 43

## Current goal
Iter 43: Phase AC Prague window stone sills — stone sill ledge (WIN_W 1.15 + 0.24 = 1.39 x SILL_H 0.09 x SILL_T 0.06) below historic windows, pressed 0.045 outside historic long facades (>=5.0) at y WIN_SILL - SILL_H/2 +0.015 (top flush WIN_SILL), deterministic per WorldSeed sill_stone per window, visual-only, stone a8a098 lerp 0.10, layer f* tagged sill; BuildingBuilder._facade_sill_ledges via WorldSeed, visual-only, no collision; citytest + smoke green (1 new assertion).
