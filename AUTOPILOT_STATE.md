...
STATUS: ACTIVE
LAST_ITER: 46
UPDATED: 2026-08-27 (JST, cron run) — iter 46

## Current goal
Iter 46: Phase AF Prague window sill corbels — stone support brackets (CORBEL_W 0.13 x CORBEL_H 0.16 x CORBEL_D 0.09) under historic sills at y WIN_SILL -SILL_H+0.015 -CORBEL_H/2 (top flush with sill bottom, t_center +/-0.35 pair), pressed 0.045 outside historic long facades (>=5.0), deterministic per WorldSeed sill_corbel per window, visual-only, stone b8a898 lerp 0.11, layer f* tagged corbel, even pairs; BuildingBuilder._facade_sill_corbels via WorldSeed, visual-only, no collision; citytest + smoke green (1 new assertion).
