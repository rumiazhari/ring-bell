...
STATUS: ACTIVE
LAST_ITER: 41
UPDATED: 2026-08-27 (JST, cron run) — iter 41

## Current goal
Iter 41: Phase AA Prague window flower boxes — terracotta trough (FLOWER_W 1.38 x FLOWER_H 0.18 x FLOWER_D 0.22) + foliage bloom (FLOWER_W*0.92 x FLOWER_BLOOM_H 0.12 x FLOWER_D*0.88) under historic sills, pressed 0.045 outside historic long facades (>=5.0) at sill height (trough WIN_SILL-FLOWER_H/2 + bloom WIN_SILL+FLOWER_BLOOM_H/2), deterministic per WorldSeed flowerbox per window, visual-only trough+bloom pair, terracotta + greens desaturated 0.18, layer f* tagged flowerbox; BuildingBuilder._facade_flower_boxes via WorldSeed, visual-only, no collision; citytest + smoke green (1 new assertion).
