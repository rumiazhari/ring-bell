...
STATUS: ACTIVE
LAST_ITER: 44
UPDATED: 2026-08-27 (JST, cron run) — iter 44

## Current goal
Iter 44: Phase AD Prague window stone jambs — vertical stone reveals (JAMB_W 0.11 x WIN_H 1.35 x JAMB_T 0.04) flanking historic windows, pressed 0.045 outside historic long facades (>=5.0) at y WIN_SILL+WIN_H/2 (sill mid), deterministic per WorldSeed window_jamb per window (left+right pair), visual-only, stone b0a898 lerp 0.11, layer f* tagged jamb; BuildingBuilder._facade_window_jambs via WorldSeed, visual-only, no collision; citytest + smoke green (1 new assertion).
