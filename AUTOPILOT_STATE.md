...
STATUS: ACTIVE
LAST_ITER: 48
UPDATED: 2026-08-27 (JST, cron run) — iter 48

## Current goal
Iter 48: Phase AH Prague facade quoins — rusticated corner stones (QUOIN_W 0.44 x QUOIN_T 0.06 x fh-0.02, 8*n per building: 2 per side per floor) pressed 0.05 outside historic corners (>=5.0 both axes), deterministic per WorldSeed quoin per building (QUOIN_PROB 0.60, 8 per floor: N wall west+east + S wall west+east + E wall north+south + W wall north+south at y f*fh+fh*0.5, visual-only thin stone blocks, stone aba090 lerp 0.11, layer f* tagged quoin); BuildingBuilder._facade_quoins via WorldSeed, visual-only, no collision; citytest + smoke green (1 new assertion).

## This iteration
[autopilot] iter 48: Phase AH Prague facade quoins rusticate historic corners with classic stone quoins: BuildingBuilder adds QUOIN_W 0.44 x QUOIN_T 0.06 x (fh-0.02) corner stones with _facade_quoins (historic-gated >=5.0 both axes, deterministic per WorldSeed quoin per building with QUOIN_PROB 0.60 — one roll per building, on success exactly 8*n boxes: 2 per side per floor at y f*fh+fh*0.5 pressed 0.05 outside wall — N wall west+east at z -QUOIN_T*0.5-0.05, S wall at d+QUOIN_T*0.5+0.05, E wall north+south at x w+QUOIN_T*0.5+0.05, W wall at -QUOIN_T*0.5-0.05, all at x/z QUOIN_W*0.5 and w/d-QUOIN_W*0.5, visual-only thin stone blocks, stone aba090 lerp 0.11, layer f* tagged quoin); citytest + smoke green (0 failures); 1 new assertion passes ("facade quoins: Prague rusticated corner stones on historic facades (deterministic)" - 0.44 x (fh-0.02) x 0.06 at corners + outside-footprint + visual-only + deterministic rebuild + 8*n count + per-floor 8 + non-historic/tiny gating + historic ring hit).

[autopilot] iter 47: Phase AG Prague doorway stone portal frames historic entrances with classic stone entrance surround: BuildingBuilder adds PORTAL_JAMB_W 0.16 x PORTAL_JAMB_H 2.25 x PORTAL_JAMB_T 0.05 jamb pair + PORTAL_LINTEL_H 0.18 x PORTAL_LINTEL_T 0.055 lintel (~2.22 width DOOR_W+2*JAMB_W+EXTRA) with _facade_door_portals (historic-gated >=5.0, deterministic per WorldSeed door_portal per building with PORTAL_PROB 0.65 — one roll per building, on success exactly 3 boxes: left+right jambs + header lintel centered on doorway length*0.5 at jamb_t mid +/- (DOOR_W/2+JAMB_W/2), jambs at y PORTAL_JAMB_H/2 and lintel at y DOOR_H+0.02+0.09, pressed 0.045 outside wall, visual-only thin stone boxes, stone b9aa90 desaturated 0.11, layer f0 tagged portal); citytest + smoke green (0 failures); 1 new assertion passes ("facade doorway portal: Prague stone jambs + lintel framing historic entrance (deterministic)" - jamb 0.16 x 2.25 x 0.05 + lintel 2.22 x 0.18 x 0.055 at doorway + outside-footprint + visual-only + deterministic rebuild + triple count + non-historic/tiny gating + historic ring hit).

[autopilot] iter 46: Phase AF Prague window sill corbels bracket historic sills with classic stone consoles: BuildingBuilder adds CORBEL_W 0.13 x CORBEL_H 0.16 x CORBEL_D 0.09 stone bracket pair with _facade_sill_corbels (historic-gated >=5.0, deterministic per WorldSeed sill_corbel per window, visual-only small blocks pressed 0.045 outside wall — corbels under sill at y WIN_SILL -SILL_H+0.015 -CORBEL_H/2 (top flush sill bottom) at t_center +/-0.35 pair inset inside window, stone b8a898 desaturated 0.11, layer f* tagged corbel, even left+right pairs); citytest + smoke green (0 failures); 1 new assertion passes ("facade window corbels: Prague stone sill corbels below historic sills (deterministic)" - 0.13 x 0.16 x 0.09 at sill underside + outside-footprint + visual-only + deterministic rebuild + even pairs + non-historic/tiny gating + historic ring hit).

[autopilot] iter 45: Phase AE Prague window stone keystones crown historic windows with classic header crowns: BuildingBuilder adds KEYSTONE_W 0.28 x KEYSTONE_H 0.18 x KEYSTONE_T 0.055 central keystone with _facade_window_keystones (historic-gated >=5.0, deterministic per WorldSeed window_keystone per window, visual-only thin block pressed 0.050 outside wall — keystone centered at t_center at y WIN_SILL+WIN_H+0.24 (just crowning the AB lintel, ~1cm below lintel top so header reads as one assembly), limestone c4b8a0 desaturated 0.11, layer f* tagged keystone); citytest + smoke green (0 failures); 1 new assertion passes ("facade window keystones: Prague stone keystones above historic windows (deterministic)" - thin 0.055 + 0.28 x 0.18 at header crown + outside-footprint + visual-only + deterministic rebuild + non-historic/tiny gating + historic ring hit).

[autopilot] iter 44: Phase AD Prague window stone jambs flank historic windows with crisp vertical reveals: BuildingBuilder adds JAMB_W 0.11 x WIN_H 1.35 x JAMB_T 0.04 stone jamb pair with _facade_window_jambs (historic-gated >=5.0, deterministic per WorldSeed window_jamb per window, visual-only thin strips pressed 0.045 outside wall — jambs at WIN_SILL+WIN_H/2 (sill mid) with inner faces flush to window edges at t_center ± (WIN_W/2+JAMB_W/2), stone b0a898 desaturated 0.11, layer f* tagged jamb, even left+right pairs); citytest + smoke green (0 failures); 1 new assertion passes ("facade window jambs: Prague stone jambs flanking historic windows (deterministic)" - thin 0.04 + 0.11 x 1.35 at window mid + outside-footprint + visual-only + deterministic rebuild + even pairs + non-historic/tiny gating + historic ring hit).

[autopilot] iter 43: Phase AC Prague window stone sills frame historic windows with crisp sill ledges: BuildingBuilder adds SILL_W 1.39 (WIN_W+0.24) x SILL_H 0.09 x SILL_T 0.06 stone sill with _facade_sill_ledges (historic-gated >=5.0, deterministic per WorldSeed sill_stone per window, visual-only thin slab pressed 0.045 outside wall — sill top flush WIN_SILL+0.015, stone a8a098 desaturated 0.10, layer f* tagged sill); citytest + smoke green (0 failures); 1 new assertion passes ("facade window sills: Prague stone sill ledges below historic windows (deterministic)" - thin 0.06 + 1.39 x 0.09 at sill height + outside-footprint + visual-only + deterministic rebuild + non-historic/tiny gating + historic ring hit).
...
26. [Done] Phase AC idea: Prague window stone sills — stone sill ledge with crisp header (WIN_W 1.15 + 0.24 = 1.39 x SILL_H 0.09 x SILL_T 0.06, 60% per historic window >=5.0), deterministic per (building,side,floor,window) via WorldSeed sill_stone, visual-only thin slab pressed 0.045 outside wall at sill line (top flush WIN_SILL+0.015), stone a8a098 desaturated 0.10, layer f* tagged sill; city-test verifies thin 0.06 + 1.39 x 0.09 at sill height + outside-footprint + visual-only + deterministic rebuild + gating.
   [COMPLETE in iter 43] SILL_T 0.06 / SILL_W 1.39 / SILL_H 0.09 / SILL_PROB 0.60 with _facade_sill_ledges (historic-gated >=5.0, deterministic per WorldSeed sill_stone per window, visual-only thin slab pressed 0.045 outside wall at sill height — sill WIN_SILL - SILL_H/2 +0.015 (top flush WIN_SILL), stone a8a098 lerp 0.10, layer f* tagged sill); citytest + smoke green (0 failures); 1 new assertion ("facade window sills: Prague stone sill ledges below historic windows (deterministic)" - thin 0.06 + 1.39 x 0.09 at sill height + outside-footprint + visual-only + deterministic rebuild + non-historic/tiny gating + historic ring hit).

27. [Done] Phase AD idea: Prague window stone jambs — vertical stone reveals flanking historic windows (JAMB_W 0.11 x WIN_H 1.35 x JAMB_T 0.04, 56% pair per historic window >=5.0), deterministic per (building,side,floor,window) via WorldSeed window_jamb, visual-only thin strips pressed 0.045 outside wall at window mid-height (WIN_SILL+WIN_H/2) with inner faces flush to window edges, stone b0a898 desaturated 0.11, layer f* tagged jamb, even left+right pairs; city-test verifies thin 0.04 + 0.11 x 1.35 at window mid + outside-footprint + visual-only + deterministic rebuild + even pairs + gating.
   [COMPLETE in iter 44] JAMB_W 0.11 / WIN_H 1.35 / JAMB_T 0.04 / JAMB_PROB 0.56 with _facade_window_jambs (historic-gated >=5.0, deterministic per WorldSeed window_jamb per window, visual-only thin strips pressed 0.045 outside wall at window mid — jambs at WIN_SILL+WIN_H/2 with inner faces at t_center ± (WIN_W/2+JAMB_W/2), stone b0a898 lerp 0.11, layer f* tagged jamb, even left+right pairs); citytest + smoke green (0 failures); 1 new assertion ("facade window jambs: Prague stone jambs flanking historic windows (deterministic)" - thin 0.04 + 0.11 x 1.35 at window mid + outside-footprint + visual-only + deterministic rebuild + even pairs + non-historic/tiny gating + historic ring hit).


28. [Done] Phase AE idea: Prague window stone keystones — central limestone keystone (KEYSTONE_W 0.28 x KEYSTONE_H 0.18 x KEYSTONE_T 0.055, 52% per historic window >=5.0) centered above historic windows at y = WIN_SILL+WIN_H+0.24 (just crowning the AB lintel, ~1cm embedded), deterministic per (building,side,floor,window) via WorldSeed window_keystone, visual-only thin block pressed 0.050 outside wall, centered at t_center, limestone c4b8a0 lerp 0.11, layer f* tagged keystone; city-test verifies thin 0.055 + 0.28 x 0.18 at header crown + outside-footprint + visual-only + deterministic rebuild + gating.
   [COMPLETE in iter 45] KEYSTONE_W 0.28 / KEYSTONE_H 0.18 / KEYSTONE_T 0.055 / KEYSTONE_PROB 0.52 with _facade_window_keystones (historic-gated >=5.0, deterministic per WorldSeed window_keystone per window, visual-only thin block pressed 0.050 outside wall at header crown — keystone centered at t_center at y WIN_SILL+WIN_H+0.24 (just crowning the AB lintel, ~1cm below lintel top), limestone c4b8a0 lerp 0.11, layer f* tagged keystone); citytest + smoke green (0 failures); 1 new assertion ("facade window keystones: Prague stone keystones above historic windows (deterministic)" - thin 0.055 + 0.28 x 0.18 at header crown + outside-footprint + visual-only + deterministic rebuild + non-historic/tiny gating + historic ring hit).

29. [Done] Phase AF idea: Prague window sill corbels — stone console brackets supporting historic sills (CORBEL_W 0.13 x CORBEL_H 0.16 x CORBEL_D 0.09, 50% pair per historic window >=5.0) under sill ledge at y = WIN_SILL -SILL_H+0.015 -CORBEL_H/2 (top flush with sill bottom, t_center +/-0.35 pair), deterministic per (building,side,floor,window) via WorldSeed sill_corbel, visual-only small blocks pressed 0.045 outside wall, stone b8a898 desaturated 0.11, layer f* tagged corbel, even left+right pairs; city-test verifies 0.13 x 0.16 x 0.09 at sill underside + outside-footprint + visual-only + deterministic rebuild + even pairs + non-historic/tiny gating + historic ring hit.
   [COMPLETE in iter 46] CORBEL_W 0.13 / CORBEL_H 0.16 / CORBEL_D 0.09 / CORBEL_PROB 0.50 with _facade_sill_corbels (historic-gated >=5.0, deterministic per WorldSeed sill_corbel per window, visual-only small blocks pressed 0.045 outside wall at sill underside — corbels at WIN_SILL -SILL_H+0.015 -CORBEL_H/2 (top flush sill bottom) at t_center +/-0.35 pair, stone b8a898 lerp 0.11, layer f* tagged corbel, even pairs); citytest + smoke green (0 failures); 1 new assertion ("facade window corbels: Prague stone sill corbels below historic sills (deterministic)" - 0.13 x 0.16 x 0.09 at sill underside + outside-footprint + visual-only + deterministic rebuild + even pairs + non-historic/tiny gating + historic ring hit).

30. [Done] Phase AG idea: Prague doorway stone portal — framed historic entrance with jamb pair (PORTAL_JAMB_W 0.16 x PORTAL_JAMB_H 2.25 x PORTAL_JAMB_T 0.05) + header lintel (DOOR_W+2*JAMB_W+0.40 ~2.22 x PORTAL_LINTEL_H 0.18 x PORTAL_LINTEL_T 0.055) at y DOOR_H+0.02+0.09, jambs at DOOR_H/2 (mid +/- DOOR_W/2+JAMB_W/2) centered on doorway length*0.5, pressed 0.045 outside historic entrance wall (>=5.0), deterministic per WorldSeed door_portal per building (PORTAL_PROB 0.65, 3-box triple: 2 jambs+1 lintel), visual-only thin stone boxes, stone b9aa90 lerp 0.11, layer f0 tagged portal; city-test verifies jamb 0.16 x 2.25 x 0.05 + lintel 2.22 x 0.18 x 0.055 at doorway + outside-footprint + visual-only + deterministic rebuild + triple count + non-historic/tiny gating + historic ring hit.
   [COMPLETE in iter 47] PORTAL_JAMB_W 0.16 / PORTAL_JAMB_H 2.25 / PORTAL_JAMB_T 0.05 / PORTAL_LINTEL_H 0.18 / PORTAL_LINTEL_T 0.055 / PORTAL_LINTEL_EXTRA 0.40 / PORTAL_PROB 0.65 with _facade_door_portals (historic-gated >=5.0, deterministic per WorldSeed door_portal per building — one roll per building, exactly 3 boxes: left+right jambs at y PORTAL_JAMB_H/2 + header lintel at y DOOR_H+0.02+0.09 centered on doorway length*0.5 at mid +/- (DOOR_W/2+JAMB_W/2), pressed 0.045 outside wall, visual-only thin stone boxes, stone b9aa90 lerp 0.11, layer f0 tagged portal); citytest + smoke green (0 failures); 1 new assertion ("facade doorway portal: Prague stone jambs + lintel framing historic entrance (deterministic)" - jamb 0.16 x 2.25 x 0.05 + lintel 2.22 x 0.18 x 0.055 at doorway + outside-footprint + visual-only + deterministic rebuild + triple count + non-historic/tiny gating + historic ring hit).

31. [Done] Phase AH idea: Prague facade quoins — rusticated corner stones (QUOIN_W 0.44 x QUOIN_T 0.06 x fh-0.02, 8*n per building: 2 per side per floor) pressed 0.05 outside historic corners (>=5.0 both axes), deterministic per WorldSeed quoin per building with QUOIN_PROB 0.60 — one roll per building, on success exactly 8*n boxes: 2 per side per floor at y f*fh+fh*0.5, visual-only thin stone blocks, stone aba090 desaturated 0.11, layer f* tagged quoin; city-test verifies 0.44 x (fh-0.02) x 0.06 at corners + outside-footprint + visual-only + deterministic rebuild + 8*n count + per-floor 8 + non-historic/tiny gating + historic ring hit.
   [COMPLETE in iter 48] QUOIN_W 0.44 / QUOIN_T 0.06 / QUOIN_MIN_SIDE 5.0 / QUOIN_PROB 0.60 with _facade_quoins (historic-gated >=5.0 both axes, deterministic per WorldSeed quoin per building — one roll per building, on success exactly 8*n boxes: 2 per side per floor at y f*fh+fh*0.5 pressed 0.05 outside wall — N wall west+east at z -QUOIN_T*0.5-0.05, S wall at d+QUOIN_T*0.5+0.05, E wall north+south at x w+QUOIN_T*0.5+0.05, W wall at -QUOIN_T*0.5-0.05, all at x/z QUOIN_W*0.5 and w/d-QUOIN_W*0.5, visual-only thin stone blocks, stone aba090 lerp 0.11, layer f* tagged quoin); citytest + smoke green (0 failures); 1 new assertion ("facade quoins: Prague rusticated corner stones on historic facades (deterministic)" - 0.44 x (fh-0.02) x 0.06 at corners + outside-footprint + visual-only + deterministic rebuild + 8*n count + per-floor 8 + non-historic/tiny gating + historic ring hit).
