...
STATUS: ACTIVE
LAST_ITER: 38
UPDATED: 2026-08-27 (JST, cron run) — iter 38

## Current goal
Iter 38: Phase X Prague facade signage — faded shop signs (0.85-1.45×0.42 at y ~2.02, 48% chance) on non-entrance historic long facades + enamel house-number plaques (0.32 square at y 1.35, 60% chance beside door) pressed as SIGNAGE_T 0.02 thin visual planes just outside the wall (visual-only, deterministic per WorldSeed house_num/shop_sign, gated historic >=5.0), BuildingBuilder._facade_signage via WorldSeed, desaturated Prague palette; citytest + smoke green (1 new assertion).
