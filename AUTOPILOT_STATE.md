...
STATUS: ACTIVE
LAST_ITER: 45
UPDATED: 2026-08-27 (JST, cron run) — iter 45

## Current goal
Iter 45: Phase AE Prague window stone keystones — central header stone (KEYSTONE_W 0.28 x KEYSTONE_H 0.18 x KEYSTONE_T 0.055) centered above historic windows at y WIN_SILL+WIN_H+0.24 (just crowning the AB lintel, ~1cm embedded), pressed 0.050 outside historic long facades (>=5.0), deterministic per WorldSeed window_keystone per window, visual-only, limestone c4b8a0 lerp 0.11, layer f* tagged keystone; BuildingBuilder._facade_window_keystones via WorldSeed, visual-only, no collision; citytest + smoke green (1 new assertion).
