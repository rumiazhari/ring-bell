...
STATUS: ACTIVE
LAST_ITER: 37
UPDATED: 2026-08-27 (JST, cron run) — iter 37

## Current goal
Iter 37: Phase W flicker/dead-lamp variant — deterministic 3.5% dead + 18% flicker (ampl 0.18, freq 3.0 Hz, phase 0-1) on historic avenue streetlamps only (gated via _lamp_is_historic mirroring CityPlan district), MeshBatcher parallel dead/flicker/phase arrays + manifest equality + _manifest_equal extended, ChunkBuilder tags OmniLights with dead_lamp / lamp_flicker+phase meta (dead stays dark at night, flicker sputters), DayNightController modulates energy via dual-sine noise (0.6*sin +0.4*sin1.73) clamped 0.55-1.40x base, gated to night and _flicker_time.
...
20. [Done] Phase W idea: flicker/dead-lamp variant — a deterministic subset of avenue streetlamps sputters (flicker ~0.18 amplitude, ~3 Hz noise) or stays dead (2-4% dark) for post-apoc horror, gated to avenue + historic; DayNightController flicker modulates energy without touching determinism; visual-only, no collision change. Gate: --citytest (flicker range / gating).
   [COMPLETE in iter 37] STREET_DEAD_PROB 0.035 / FLICKER_PROB 0.18 / AMPL 0.18 / FREQ 3.0 on historic lamps via _lamp_is_historic (mirrors CityPlan district), MeshBatcher dead/flicker/phase parallel arrays + manifest keys + _manifest_equal determinism, ChunkBuilder OmniLights tagged dead_lamp (visible false always) or lamp_flicker+flicker_phase (still counted in stats), DayNightController _flicker_time dual-sine noise (0.6*sin+0.4*sin1.73) clamped 0.55-1.40x base, night-only, dead stays dark; citytest + smoke green (0 failures); 1 new assertion ("flicker/dead lamps: historic subset sputters or stays dark (deterministic)" - dead 3.5%+flicker 18% in 169-chunk ring, historic gating, outer ~0, deterministic rebuild, manifest keys, node meta/phase, DNC night/day flicker modulation and dead stays dark).
