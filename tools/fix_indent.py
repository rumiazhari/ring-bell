...
# Fix indentation: the asset block currently has one extra leading tab.
# Replace the block with correctly indented version.
...
\t\t\t\t\telif not has_asset and b.asset_instance_count() == 0 and pr == parts[0]:
...
if old in text:
    ...
    import re
    # find snippet
    idx = text.find("# G9 M2 Asset Pipeline")
    print(repr(text[idx-200:idx+500]))
